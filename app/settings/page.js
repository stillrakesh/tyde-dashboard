...
export default function SettingsPage() {
  ...
  const [apiKey, setApiKey] = useState('tyde_live_98a7f10b2c3d4e5f6g7h');

  useEffect(() => {
    const accountStr = localStorage.getItem('tyde_cloud_account');
    if (accountStr) {
      try {
        ...
        console.error('Failed to parse account from localStorage', err);
      }
    }
  }, []);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px', maxWidth: '800px' }}>
      ...
      <div style={{ background: 'white', padding: '32px', borderRadius: '24px', border: '1px solid #e2e8f0' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '12px', color: '#64748b', fontWeight: '500', marginTop: '2px' }}>Use this key in your local POS Settings → Server tab to authorize cloud sync</p>
          </div>
        </div>
      ...
      <div style={{ background: 'white', padding: '32px', borderRadius: '24px', border: '1px solid #e2e8f0' }}>
        ...
        <ol style={{ paddingLeft: '20px', display: 'flex', flexDirection: 'column', gap: '14px', fontSize: '14px', color: '#334155', lineHeight: 1.6, fontWeight: '600' }}>
          ...
          <li>Go to <strong>Settings</strong> → <strong>Server tab</strong>.</li>
          <li>Locate the <strong>Cloud Dashboard API Key</strong> field.</li>
          <li>Paste your key: <code style={{ background: '#f1f5f9', padding: '2px 8px', borderRadius: '6px', fontSize: '12px', color: '#94161c' }}>{apiKey}</code></li>
